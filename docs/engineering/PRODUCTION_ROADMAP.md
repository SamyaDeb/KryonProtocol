...
| 🔴 | Tunnel hostnames | `kryon-protocol/infra/a1flex/cloudflared-config.yml` (`kryonprotocol.live`, `ws.`, `ws-testnet.`) | Running the tunnel **hijacks the other product's domain** | Replace with new hostnames and a new tunnel ID/credentials |
...
| 🟠 | Docs/app URLs | `docs/docusaurus.config.ts` (`client-eight-mu-71.vercel.app`, tagline), README links and "live on…" claims, `check-go-live.sh` | Public confusion, broken links | New domain everywhere |
